...
""" unittest for 2x1 ADD case """

from myhdl import *
from example import pipe_2x1_alu
from unittest import TestCase
import unittest

class TestPipe2x1ALUProperties(TestCase):
	...
	def tst_pipe_2x1_add(clk, srst, g, a, b, c, d, en_i, en_o):
	    """ DUT 2x1parl_add op=ADD """
	    expected = []
	    actual = []

	    @always(clk.negedge)
	    def tst_sequence():
		if en_o == pipe_2x1_alu.OOK:
		    actual.append(int(g))

	    	if en_o == pipe_2x1_alu.OIDLE and not srst:
	...
	nstat = Signal(intbv(0, min=0, max=3))
   	...
   	pipe_2x1_alu.DEBUG = True

	# start simulation and link
    	dut_2x1_0 = pipe_2x1_alu.pipe_2x1_alu(\
    		clk=clk, \
    		srst=srst, \
    		g=g, \
    		a=a, \
    	...
    	tst_sys_0 = tst_sys(\
    		clk=clk, \
    		srst=srst)
    	tst_2x1_0 = tst_pipe_2x1_add(clk=clk, \
    		srst=srst, \
    		g=g, \
    		a=a, \
    		b=b, \
	...
	# trace signal, vcd dump
    	traceSignals(pipe_2x1_alu.pipe_2x1_alu, \
    		clk=clk, \
    		srst=srst,\
    		g=g, \
    		a=a, \
    	...
    	sim = Simulation(dut_2x1_0, tst_sys_0, tst_2x1_0)
    	sim.run(quiet=1)
	...
	toVerilog(pipe_2x1_alu.pipe_2x1_alu, \
		clk=clk, \
		srst=srst, \
		g=g, \
		a=a, \
